...
class Farm
  ...
  def verify_food
    loop do
      ...
      if self.food_info.has_key?(field_type)
        return field_type
      elsif field_type.to_i > 0 && field_type.to_i < self.food_info.length + 1
  ...
  # Makes sure the size of the field is greater than 0. Will return nil if input left blank.
  def size_field
    loop do
  ...
  def harvest
    ...
    self.fields.each do |field|
      harvest_yield = field.field_size * self.food_info[field.name]
